...
# GNU timeout(1) exits 124 when its deadline fires. Python may instead see a
# negative signal when the wrapper itself is killed after --kill-after.
# SIGABRT is only a timeout when ForteSMT printed its SIGALRM marker.
...
def recorded_result_row(row: Mapping[str, str | None], timeout: float) -> str:
    """Return the normalized process-level result for one persisted result row."""
